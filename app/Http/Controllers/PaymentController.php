...
//        curl_setopt($curl, CURLOPT_USERPWD, $consumerKey . ':' . $consumerSecret);
    ...
    public function createXmlObject()
    {
        ...
        //TRANSACTION LEVEL
        $child_node_amount = $dom->createElement('PaymentAmount', '300');
        $transaction_node->appendChild($child_node_amount);

        $child_node_currency = $dom->createElement('PaymentCurrency', 'KSH');
        $transaction_node->appendChild($child_node_currency);

        $child_node_ref = $dom->createElement('CompanyRef', 'The Campaign');
        ...
        $second_level_child_node_service_type = $dom->createElement('ServiceType', 45);
        ...
        $second_level_child_node_service_date = $dom->createElement('ServiceDate', 'Flight from Nairobi to Diani');
